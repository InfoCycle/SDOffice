	...
	$.fn.menu.methods = {
		...
		getItem: function(jq, itemEl){
			...
			var item = {
				...
				href: t.attr('href'),
				name: t.attr('name'),
				//isLeaf: t.attr('isLeaf'),
			    //isCheck: t.attr('isCheck'),
...
//下拉菜单创建及赋值
			...
			if(type=="m"){
				if(valueold.length>1)
				{
					...
					if(valuetemp.length==i)
						...
						$("#"+id).val(($("#"+id).val().toString().replace(valuetemp[i],',')).replace(',,',''));
				}else
					$("#"+id).val(value);
				//当为','则清空
				if($("#"+id).val()==",")
       ...
       * dataurl:获取数据url
       * datajson:获取数据参数
       * fieldid:存放Value的元素Id
       * fieldname:存放Name的元素Id
       * butnid:菜单绑定的元素Id
       * multiSelect:”s“为单选，”m“为多选
       */
      function CreateSelectMenu(dataurl,datajson,fieldid,fieldname,butnid,multiSelect){
				 var objMenu;
				 $.post(dataurl, datajson, function(data) {
					 objMenu = bulidMenu(data,{
		  		        onClick:function(item){
						 	//为单选
		  		        	if(multiSelect=="s")
			  		        {
			  		        	if($("#"+fieldid).val()!=""){
			  		        		objMenu.menu('setIcon', {
						  		    	target: $('#'+$("#"+fieldid).val())[0],
						  		    	iconCls: 'icon-leaf'
						  		    });
				        		}
			  		        }
			  		        ...
			  		        if(multiSelect=="m")
			  		        {
			  		        	fieldidvalue=$("#"+fieldid).val().toString();
			  		        	
			  		        	if(fieldidvalue!="")
			  		        	{
				  		        	fieldidarray=fieldidvalue.split(",");
						 			//循环设置
				  		        	for(i=0;i<fieldidarray.length;i++)
				  		        	{
				  		        		objMenu.menu('setIcon', {
							  		    	target: $('#'+fieldidarray[i])[0],
							  		    	iconCls: 'icon-leaf'
							  		    });
					        		}
			  		        	}
		  		        	...
		  		        	if(item.href=="#Y")
		  		        	 	{	  		        			
		  		        			setValue(fieldid,item.id,multiSelect);
		  		        			setValue(fieldname,item.text,multiSelect);
		  		        			if(multiSelect=="s")
					  		        {
					  		        	if($("#"+fieldid).val()!=""){
					  		        		objMenu.menu('setIcon', {
								  		    	target: $('#'+$("#"+fieldid).val())[0],
								  		    	iconCls: 'icon-menu-selected'
								  		    });
						        		}
					  		        }
		  		        			 if(multiSelect=="m")
